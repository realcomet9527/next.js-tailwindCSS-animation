...
const Valuepart1 = () => {
  ...
  const handleUpdate = () => {
    ...
    if(text.heading!==""&&text.para!==""&&text.color!==""){
      axios
      .post(`${serverLink}value/update/1`, text)
      .then((response1) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${serverLink}value/part/1`) // Change the URL accordingly
      .then((response) => {
    ...
    <DashboardLayout>
      <div className="p-10 relative h-[80vh] w-full flex">
        <div className="w-1/3 border-r-[1px]">
          <h1 className="text-2xl pb-4 font-bold">Values Part 1</h1>
