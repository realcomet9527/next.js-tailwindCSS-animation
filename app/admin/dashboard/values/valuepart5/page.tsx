...
const Valuepart5 = () => {
  ...
  const handleUpdate = () => {
    ...
    if(text.heading!==""&&text.para!==""&&text.color!==""){
      axios
      .post(`${serverLink}value/update/5`, text)
      .then((response1) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${serverLink}value/part/5`) // Change the URL accordingly
      .then((response) => {
    ...
    <DashboardLayout>
      <div className="p-10 relative h-[80vh] w-full flex">
        <div className="w-1/3 border-r-[1px]">
          <h1 className="text-2xl pb-4 font-bold">Values Part 5</h1>
