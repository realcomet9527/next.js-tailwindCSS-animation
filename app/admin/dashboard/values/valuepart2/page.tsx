...
const Valuepart2 = () => {
  ...
  const handleUpdate = () => {
    ...
    if(text.heading!==""&&text.para!==""&&text.color!==""){
      axios
      .post(`${serverLink}value/update/2`, text)
      .then((response1) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${serverLink}value/part/2`) // Change the URL accordingly
      .then((response) => {
    ...
    <DashboardLayout>
      <div className="p-10 relative h-[80vh] w-full flex">
        <div className="w-1/3 border-r-[1px]">
          <h1 className="text-2xl pb-4 font-bold">Values Part 2</h1>
