...
const Valuepart3 = () => {
  ...
  const handleUpdate = () => {
    ...
    if(text.heading!==""&&text.para!==""&&text.color!==""){
      axios
      .post(`${serverLink}value/update/3`, text)
      .then((response1) => {
        setAlert('Value Part Updated')
  ...
  useEffect(() => {
    ...
    axios
      .get(`${serverLink}value/part/3`) // Change the URL accordingly
      .then((response) => {
    ...
    <DashboardLayout>
      <div className="p-10 relative h-[80vh] w-full flex">
        <div className="w-1/3 border-r-[1px]">
          <h1 className="text-2xl pb-4 font-bold">Values Part 3</h1>
          ...
          <textarea
            ...
            onChange={handleTextChange}
            placeholder="Enter text"
            className="w-[90%] border rounded-md p-2"></textarea>
        ...
        <div className="CurrentSection w-1/3 px-10">
          ...
          {sectionData &&
            sectionData.map((item: any, index: number) => {
              return (
                <div key={index}>
                    <h1 className="text-3xl my-2 text-ubuntu-bold">{item.heading}</h1>
