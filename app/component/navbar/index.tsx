...
import { AssignCir2False, AssignCir2True,AssignIndCirFalse,AssignIndCirTrue, AssignCirFalse, AssignCirTrue, AssignFalse, AssignTrue, AssignZero } from "@/lib/store/slices/Authslice";
...
// console.log(sectionData, 'sectionData');
  ...
  const handleBar = () => {
    ...
    if (check) {
      ...
      dispatch(AssignIndCirTrue())
      setTimeout(() => {
        dispatch(AssignCir2True())
        dispatch(AssignIndCirFalse())
  ...
  return (
    <>
      <div
        className="bg-transparent sm:pt-12 pt-10 top-0 left-0 h-40 w-full fixed flex
    justify-between z-[9999]"
      >
        <div onClick={()=> dispatch(AssignZero())} className="logo sm:h-3/4 h-2/5 sm:ml-10 ml-6 sm:mt-[-10px] -mt-2">
          <a href="/">
            <Image width={100} height={50} priority={true} src={sectionData.logoImage || '/images/logo.png'} alt="Logo.jpg" />
          </a>
        </div>
        <div>
          <div
